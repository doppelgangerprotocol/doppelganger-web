...
Security guarantees tested:
  ...
  - alice_pubkey_jwk stored in Redis but never returned in GET response
  - Session created with correct initial phase
  - Bad input rejected before hitting Redis or embedding service
"""

import json
import pytest
from unittest.mock import patch
from tests.conftest import ALICE_VEC
...
def post_session(client, payload=None):
...
class TestSessionCreation:
    ...
    def test_returns_201(self, client):
        assert post_session(client).status_code == 201

    def test_returns_session_id(self, client):
        data = post_session(client).get_json()
    ...
    def test_returns_qr_code(self, client):
        data = post_session(client).get_json()
    ...
    def test_session_stored_in_redis(self, client, fake_redis):
        data = post_session(client).get_json()
        stored = fake_redis.hgetall(f"session:{data['session_id']}")
        assert stored != {}
    ...
    def test_embedding_stored_as_vector(self, client, fake_redis):
        """answer_embedding should be a JSON vector, not text."""
        data = post_session(client).get_json()
        stored = fake_redis.hgetall(f"session:{data['session_id']}")
...
class TestGetSession:
    ...
    def test_returns_question_and_phase(self, client):
        session_id = post_session(client).get_json()["session_id"]
    ...
    def test_alice_pubkey_absent_from_get_response(self, client):
    ...
    def test_answer_embedding_absent_from_get_response(self, client):
        """Embedding vector should not be exposed to Bob."""
        session_id = post_session(client).get_json()["session_id"]
        resp = client.get(f"/api/session/{session_id}")
        assert "answer_embedding" not in resp.get_json()
...
class TestSessionValidation:
    ...
    def test_missing_alice_name(self, client):
        resp = post_session(client, {**VALID_PAYLOAD, "alice_name": ""})
        assert resp.status_code == 400
        assert "alice_name" in resp.get_json()["error"]

    def test_missing_memory_question(self, client):
        resp = post_session(client, {**VALID_PAYLOAD, "memory_question": ""})
        assert resp.status_code == 400

    def test_missing_memory_answer(self, client):
        resp = post_session(client, {**VALID_PAYLOAD, "memory_answer": ""})
        assert resp.status_code == 400

    def test_question_too_long(self, client):
        resp = post_session(client, {**VALID_PAYLOAD, "memory_question": "x" * 501})
        assert resp.status_code == 400

    def test_answer_too_long(self, client):
        resp = post_session(client, {**VALID_PAYLOAD, "memory_answer": "x" * 501})
        assert resp.status_code == 400

    def test_invalid_pubkey_not_ec(self, client):
        resp = post_session(client, {**VALID_PAYLOAD,
                                     "alice_pubkey_jwk": {"kty": "RSA", "crv": "P-256"}})
        assert resp.status_code == 400

    def test_invalid_pubkey_wrong_curve(self, client):
        resp = post_session(client, {**VALID_PAYLOAD,
                                     "alice_pubkey_jwk": {"kty": "EC", "crv": "P-384"}})
        assert resp.status_code == 400

    def test_pubkey_not_a_dict(self, client):
        resp = post_session(client, {**VALID_PAYLOAD, "alice_pubkey_jwk": "not-a-jwk"})
        assert resp.status_code == 400
